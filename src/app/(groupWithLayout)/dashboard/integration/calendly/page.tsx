...
const page = () => {
    ...
    const checkConnection = async () => {
        try {
            const response = await fetch('/api/integration/calendly/events');
            if (response.ok) {
                setConnected(true);
            } else {
                setConnected(false);
    ...
    const disconnectCalendar = async () => {
        ...
        try {
            const response = await fetch('/api/integration/calendly/disconnect', {
                method: 'DELETE',
            });
    ...
    return (
        <div className="p-6 space-y-6">
            <div className="text-2xl font-bold">Calendly Integration</div>
            
            {!connected ? (
                <div className="bg-white rounded-lg border border-gray-200 shadow-sm p-6 text-center">
                    <Calendar className="w-12 h-12 text-gray-400 mx-auto mb-4" />
                    <h3 className="text-lg font-semibold text-gray-900 mb-2">Calendly Not Connected</h3>
                    <p className="text-gray-600 mb-4">Connect your Calendly to view and manage scheduled events.</p>
                    ...
                    <div className="bg-green-50 rounded-lg border border-green-200 shadow-sm p-4">
                        <div className="flex items-center justify-between">
                            <div className="flex items-center space-x-2">
                                ...
                                <span className="text-green-800 font-medium">Calendly Connected</span>
                            </div>
                            <Button
                        ...
                        <div className="p-6 border-b border-gray-200">
                            <div className="flex items-center justify-between">
                                <div>
                                    ...
                                    <p className="text-sm text-gray-600 mt-1">Your upcoming Calendly events</p>
                                </div>
                                    ...
                                    {events.map((event, index) => (
                                        <div key={index} className="border border-gray-200 rounded-lg p-4">
                                            ...
                                            {event.event_type?.name && (
                                                <p className="text-sm text-gray-600 mt-1">{event.event_type.name}</p>
                                            )}
