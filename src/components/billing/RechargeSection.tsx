...
const RechargeSection = () => {
  ...
  return (
    <Card className="h-full bg-white dark:bg-gray-950 border-gray-200 dark:border-gray-700" id="recharge">
      <CardHeader className="pb-2">
        <CardTitle className="text-xl font-semibold text-gray-800 dark:text-gray-100">Recharge Account</CardTitle>
      </CardHeader>
      <CardContent>
        <div className="space-y-4">
          <div className="grid grid-cols-3 gap-3">
            {RECHARGE_OPTIONS.map(option => (
              <Button
                key={option.value}
                variant={selectedAmount === option.value ? 'default' : 'secondary'}
                className={`
                  w-full h-12 text-base font-medium 
                  ${selectedAmount === option.value ? 'ring-2 ring-indigo-300 dark:ring-indigo-500' : ''}
                `}
