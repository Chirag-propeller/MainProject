...
import { Upload, FileText, Table, Phone } from 'lucide-react';
...
const RecipientsTab: React.FC<RecipientsTabProps> = ({ 
  ...
  // CSV Upload states
  const [fileName, setFileName] = useState<string | null>(campaign.recipientFileName || null);
  ...
  const [selectedSheet, setSelectedSheet] = useState(campaign.recipientFileId || "");
  const [selectedSheetName, setSelectedSheetName] = useState(campaign.recipientFileName || "");
    ...
    if (file) {
      ...
      Papa.parse(file, {
        ...
        complete: (results) => {
          ...
          console.log('💾 Updating campaign with CSV recipients:', updatedCampaign);
  ...
  const readSheet = async () => {
    ...
    try {
      ...
      if (data.values && data.values.length > 0) {
        ...
        const selectedSheetObj = sheets.find((sheet: any) => sheet.id === selectedSheet);
        ...
        toast.success(`Successfully imported ${phoneNumbers.length} phone numbers from Google Sheet`);
      }
    } catch (error: any) {
        if(error.status === 404){
            setGoogleSheetIntegrated(false);
  ...
  const extractPhoneNumbersFromSheet = (data: any[][]) => {
    ...
    return extractPhoneNumbers(data.slice(1).map(row => {
      const obj: any = {};
  ...
  useEffect(() => {
    ...
    if (campaign.recipients && campaign.recipients.length > 0) {
      if (campaign.recipientFileProvider === 'csv' || !campaign.recipientFileProvider) {
  ...
  useEffect(() => {
    if (campaign.recipientFileProvider && campaign.recipientFileProvider !== activeTab) {
  ...
  const getCurrentPhoneNumbers = () => {
    ...
    if (provider === 'csv') {
      return extractedPhones.length > 0 ? extractedPhones : (campaign.recipients || []);
    } else {
      return sheetPhoneNumbers.length > 0 ? sheetPhoneNumbers : (campaign.recipients || []);
    }
  };
