...
const Analytics = ({
  ...
  const fetchCampaignAnalytics = async () => {
    ...
    const pendingCalls =
      ...
      (campaignData.completed_calls || 0) -
      (campaignData.failed_calls || 0);
  ...
  return (
    <div className="space-y-4 p-4 pt-0 dark:bg-gray-900 dark:text-indigo-300">
      {/* Header */}
      <div className="flex flex-row justify-between items-center sticky top-0 bg-white dark:bg-gray-900 z-10 pt-2 pb-1">
        <div>
          <h1 className="text-lg font-medium text-gray-800 dark:text-indigo-300">
            Live Analytics
          </h1>
          <p className="text-xs text-gray-500 dark:text-indigo-300">
            Last updated: {lastUpdated?.toLocaleTimeString() || "--"} •
            Auto-refresh: {isLive ? "60s" : "Off"}
          </p>
        </div>
        <Button
          className="rounded-lg text-xs dark:bg-gray-800 dark:text-indigo-300 dark:border-gray-700"
      ...
      <div className="pt-4">
        <h2 className="text-base font-medium text-gray-800 dark:text-indigo-300 mb-3">
          Follow Up Calls
        </h2>
